<!doctype html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Attorneyster-Home</title>
    <link rel="shortcut icon" href="./images/favicon.png">
    <link rel="stylesheet" href="./css/style.css">
    <link rel="stylesheet" href="./css/media.css">
    <script src="./js/setting.js"></script>
</head>

<body>
    <!-- header start -->
    <section class='header-wrap'>
        <header class='header'>
            <div class="container">
                <div class='header-main'>
                    <div class='logo'>
                        <a href="index.php" targer="_self">
                            <img src="./images/attorneystar.png" alt="Attorneyster" width='398' height='79'>
                        </a>
                    </div>
                    <div class="header-right">
                        <div class="header-cart">
                            <div class="cart-img">
                                <img src="./images/cart.png" alt="cart" width='32' height='34'>
                            </div>
                            <p>
                                Cart (0)
                            </p>
                        </div>
                        <div class="cta-btn">
                            <a href='contact.php' target='_self'>
                                Book a Consultation
                            </a>
                        </div>
                    </div>
                </div>

                <div class="header-nav">
                    <nav class='header-links'>
                        <ul>
                            <li>
                                <a href="index.php" target='_self'>
                                    Home
                                </a>
                            </li>
                            <li>
                                <a href="about-us.php" target='_self'>
                                    About us
                                </a>
                            </li>
                            <li class='header-pages'>
                                <a target='_self'>
                                    Pages
                                    <img src='./images/page-arrow.png' alt='dropdown-menu'>
                                </a>
                                <div class='header-page-content'>
                                    <a href='practice-area.php' target='_self'>Practice Area</a>
                                    <a href='pricing.php' target='_self'>Pricing</a>
                                    <a href='case-result.php' target='_self'>Cases</a>
                                    <a href='blog.php' target='_self'>Blog</a>
                                    <a href='testimonials.php' target='_self'>Testimonials</a>
                                </div>
                            </li>
                            <li>
                                <a href="contact.php" target='_self'>
                                    Contact Us
                                </a>
                            </li>
                        </ul>
                    </nav>

                    <div class="mobile-menu">
                        <img src="./images/openbtn.png" alt="open-Button" width='24' height='28'>
                    </div>
                    <div class="mobile-nav">
                        <nav>
                            <div class="mobile-menu-link">
                                <a href="index.php" target='_self'>Home</a>
                            </div>
                            <div class="mobile-menu-link">
                                <a href="about-us.php" target='_self'>About us </a>
                            </div>
                            <div class="mobile-menu-link pages">
                                <a target='_self'>
                                    Pages
                                    <img src='./images/page-arrow.png' alt='dropdown-menu'>
                                </a>
                                <div class='mobile-page-content'>
                                    <a href='practice-area.php' target='_self'>Practice Area</a>
                                    <a href='pricing.php' target='_self'>Pricing</a>
                                    <a href='case-result.php' target='_self'>Cases</a>
                                    <a href='blog.php' target='_self'>Blog</a>
                                    <a href='testimonials.php' target='_self'>Testimonials</a>
                                </div>
                            </div>
                            <div class="mobile-menu-link">
                                <a href="contact.php" target='_self'>Contact Us</a>
                            </div>
                        </nav>
                    </div>

                    <div class="header-contact-wrap">
                        <div class="nav-area">
                            <div class="nav-img-wrap">
                                <img src="./images/call.png" alt="Call-us" width='17' height='17'>
                            </div>
                            <div class="nav-contact">
                                <div class='contact-wrap'>
                                    Call Us On:
                                </div>
                                <a href="tel: 987654321" target='_self'>
                                    911-987654321
                                </a>
                            </div>
                        </div>

                        <div class="nav-area">
                            <div class="nav-img-wrap">
                                <img src="./images/email.png" alt="Email" width='18' height='14'>
                            </div>
                            <div class="nav-contact">
                                <div class='contact-wrap'>
                                    Email Us:
                                </div>
                                <a href="mailto:yourmail@mail.com" target='_self'>
                                    yourmail@mail.com
                                </a>
                            </div>
                        </div>
                    </div>
        </header>
    </section>
    <!-- header end -->

    <!-- banner start -->
    <section class='banner-wrap' style='background-image:url("./images/hero-overlay.png");'>
        <div class='header-overlay'></div>
        <div class="container">
            <div class='banner'>
                <div class="banner-left">
                    <div class="banner-content">
                        <img src="./images/practice-icon.png" alt="section-icon" width='95' height='44'>
                        <div class="banner-title">
                            <h1 class="h1">
                                Deeper Understanding Better Solutions
                            </h1>
                            <p>
                                It is a long established fact that a reader will be distracted by
                                the readable content of a looking at its layout.
                            </p>
                        </div>
                        <div class='cta-btn btn'>
                            <a href="about-us.php" target='_self'>
                                Get In Touch
                            </a>
                        </div>
                    </div>
                </div>
                <div class="banner-right">
                    <img src="./images/header-banner.png" alt="header-main" width='786' height='832'>
                </div>
            </div>
        </div>
    </section>
    <!-- banner end -->


    <!-- intro-section start -->
    <section class="intro-section-wrap">
        <div class="container">
            <div class="intro-section">
                <div class="intro-left">
                    <div class="intro-left-image">
                        <img src="./images/intro-second.png" alt="Introduction-image" width='467' height='292'>
                    </div>
                    <div class="intro-left-image">
                        <img src="./images/intro-one.png" alt="Introduction-image" width='467' height='292'>
                    </div>

                </div>

                <div class="intro-right">
                    <div class="section-title">
                        <h2 class="h2">
                            The Simple Choice
                            <br>
                            for Complex Litigation
                        </h2>
                        <img src="./images/practice-icon.png" alt="practice-section-logo" width='95' height='44'>
                    </div>

                    <div class="intro-main-content">
                        <div class="intro-content">
                            <p>
                                There are many variations of passages of Lorem Ipsum available, but the majority have su
                                alteration in some form, by injected humour, oir randomised workds which don't look even
                                slightly believable. If you are going to use a passage of Lorem Ipsum, you need to be
                                sure there isn't anything.
                            </p>
                        </div>
                        <div class="intro-details">
                            <div class="intro-image">
                                <img src="./images/intro-person.png" alt="person" width='86' height='86'>
                            </div>
                            <div class="intro-name">
                                <div class='author-name'>
                                    Peeter Park
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="intro-main">
                        <div class="intro-main-wrap">
                            <div class="intro-success">
                                <div class="intro-success-icon">
                                    <img src='./images/case-sucess.png' alt='case-sucess-img' width='56px' height='53px' />
                                </div>
                                <div class="intro-sucess-percentage">
                                    <div class='intro-count'>
                                        95%
                                        <br>
                                        Case Success
                                    </div>
                                </div>
                            </div>


                            <div class="intro-exp">
                                <div class="intro-exp-wrap">
                                    <div class="intro-exp-number">
                                        <p>35+</p>
                                    </div>
                                </div>
                                <div class="intro-exp-years">
                                    <div class='intro-count'>
                                        Years
                                        <br>
                                        Experiance
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- intro-section end -->

    <!-- practice-area-start -->
    <section class="practice-area-section-wrap">
        <div class="container">
            <div class="area-nav-section">
                <div class="practice-area-nav">
                    <div class="section-title">
                        <h2 class="h2">Explore <br> Our Practice Arease</h2>
                        <img src="./images/practice-icon.png" alt="practice-section-logo" width='95' height='44'>
                    </div>
                    <div class="practice-nav-line">

                    </div>
                    <div class="practice-nav-description">
                        <p>
                            There are <span> many variations of passages</span> of Lorem Ipsum available, but the
                            majority have su alteration in some form, by injected humour, oir <span>randomised
                                workds</span> which don't look even slightly believable.
                        </p>
                    </div>
                </div>
            </div>
            <ul class="practice-listing">
                <li>
                    <div class="practice-area-content">
                        <div class="area-content">
                            <h3 class="h3">
                                Business Law
                            </h3>
                            <p>
                                There are many variations of passages
                                of Lorem Ipsum available, but majority
                                going to use a passage.
                            </p>
                        </div>
                        <div class="practice-social">
                            <div class="practice-logo">
                                <img src="./images/bag-icon.png" alt="Business-law" width='55' height='48'>
                            </div>
                            <div class='cta-btn sm-btn'>
                                <a href='business-law.php' target='_self'>
                                    Learn More
                                </a>
                            </div>
                        </div>
                    </div>
                </li>

                <li>
                    <div class="practice-area-content">
                        <div class="area-content">
                            <h3 class="h3">
                                Construction Law
                            </h3>
                            <p>
                                There are many variations of passages
                                of Lorem Ipsum available, but majority
                                going to use a passage.
                            </p>
                        </div>
                        <div class="practice-social">
                            <div class="practice-logo">
                                <img src="./images/construction-icon.png" alt="Construction-Law" width='63' height='55'>
                            </div>
                            <div class='cta-btn sm-btn'>
                                <a href='practice-area.php' target='_self'>
                                    Learn More
                                </a>
                            </div>
                        </div>
                    </div>
                </li>

                <li>
                    <div class="practice-area-content">
                        <div class="area-content">
                            <h3 class="h3">
                                Car Accident
                            </h3>
                            <p>
                                There are many variations of passages
                                of Lorem Ipsum available, but majority
                                going to use a passage.
                            </p>
                        </div>
                        <div class="practice-social">
                            <div class="practice-logo">
                                <img src="./images/car-icon.png" alt="Car-Accident" width='70' height='44'>
                            </div>
                            <div class='cta-btn sm-btn'>
                                <a href='cases-single.php' target='_self'>
                                    Learn More
                                </a>
                            </div>
                        </div>
                    </div>
                </li>

                <li>
                    <div class="practice-area-content">
                        <div class="area-content">
                            <h3 class="h3">
                                Wrongful Death
                            </h3>
                            <p>
                                There are many variations of passages
                                of Lorem Ipsum available, but majority
                                going to use a passage.
                            </p>
                        </div>
                        <div class="practice-social">
                            <div class="practice-logo">
                                <img src="./images/death-icon.png" alt="Wrongful-Death" width='53' height='64'>
                            </div>
                            <div class='cta-btn sm-btn'>
                                <a href='practice-area.php' target='_self'>
                                    Learn More
                                </a>
                            </div>
                        </div>
                    </div>
                </li>

                <li>
                    <div class="practice-area-content">
                        <div class="area-content">
                            <h3 class="h3">
                                Criminal Law
                            </h3>
                            <p>
                                There are many variations of passages
                                of Lorem Ipsum available, but majority
                                going to use a passage.
                            </p>
                        </div>
                        <div class="practice-social">
                            <div class="practice-logo">
                                <img src="./images/law-icon.png" alt="Criminal-Law" width='53' height='55'>
                            </div>
                            <div class='cta-btn sm-btn'>
                                <a href='practice-area.php' target='_self'>
                                    Learn More
                                </a>
                            </div>
                        </div>
                    </div>
                </li>

                <li>
                    <div class="practice-area-content">
                        <div class="area-content">
                            <h3 class="h3">
                                Family Law
                            </h3>
                            <p>
                                There are many variations of passages
                                of Lorem Ipsum available, but majority
                                going to use a passage.
                            </p>
                        </div>
                        <div class="practice-social">
                            <div class="practice-logo">
                                <img src="./images/family-icon.png" alt="Family-Law" width='56' height='41'>
                            </div>
                            <div class='cta-btn sm-btn'>
                                <a href='practice-area.php' target='_self'>
                                    Learn More
                                </a>
                            </div>
                        </div>
                    </div>
                </li>
            </ul>

        </div>
    </section>
    <!-- practice-area-section end -->

    <!-- help-section start -->
    <section class="help-section-wrap">
        <div class="container">
            <div class='help-section'>
                <div class="section-title">
                    <h2 class="h2">
                        If you`re in trouble
                        <br>
                        we can help
                    </h2>
                    <img src="./images/Icon.png" alt="section-image" width='95' height='44'>
                </div>

                <div class="section-content">
                    <p>
                        There are of Lorem Ipsum available, but the majority have su alteration in some
                        <br>
                        form, by injected humour, oir which don`t look even slightly believable
                    </p>
                </div>


                <div class="help-main-wrap">
                    <div class="help-main">
                        <div class="help-item">

                            <div class="help-item-count">
                                <svg width="22" height="20" viewBox="0 0 22 20" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M11 20C11.3397 19.9996 11.6713 19.8985 11.9519 19.7099C16.1079 16.9312 17.9074 15.026 18.9 13.8349C21.0153 11.2958 22.028 8.68906 21.9994 5.86615C21.9661 2.63125 19.331 0 16.1253 0C13.7943 0 12.1798 1.29323 11.2396 2.37031C11.2098 2.40409 11.173 2.43117 11.1316 2.44973C11.0903 2.46829 11.0454 2.4779 11 2.4779C10.9546 2.4779 10.9097 2.46829 10.8684 2.44973C10.827 2.43117 10.7902 2.40409 10.7604 2.37031C9.82021 1.29219 8.20572 0 5.87469 0C2.669 0 0.0339012 2.63125 0.000585556 5.86667C-0.0279713 8.6901 0.985773 11.2969 3.09999 13.8354C4.09258 15.0266 5.89215 16.9318 10.0481 19.7104C10.3287 19.8988 10.6604 19.9997 11 20Z"
                                        fill="#D1B06B" />
                                </svg>
                                <p>
                                    7230
                                </p>
                            </div>
                        </div>
                        <div class="help-item-content">
                            <h3 class="h3">
                                Trusted Clients
                            </h3>
                            <p>
                                There are of Lorem Ipsum
                            </p>
                        </div>
                    </div>
                    <div class="help-main">
                        <div class="help-item">
                            <div class="help-item-count">
                                <svg width="26" height="24" viewBox="0 0 26 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M13 0L15.9187 8.98278H25.3637L17.7225 14.5344L20.6412 23.5172L13 17.9656L5.35879 23.5172L8.27747 14.5344L0.636266 8.98278H10.0813L13 0Z"
                                        fill="#D1B06B" />
                                </svg>
                                <p>
                                    98%
                                </p>
                            </div>

                        </div>
                        <div class="help-item-content">
                            <h3 class="h3">
                                Successful Cases
                            </h3>
                            <p>
                                There are of Lorem Ipsum
                            </p>
                        </div>
                    </div>

                    <div class="help-bg">
                        <div class="help-count">
                            15,890+
                        </div>
                        <p>
                            Criminal Defense
                            <br>
                            Case Served
                        </p>

                    </div>
                </div>
                <div class="meet-partner">
                    <div class="meet-partner-title">
                        <div class="section-title">
                            <h2 class="h2">
                                Meet The Partners
                            </h2>
                        </div>
                    </div>

                    <div class="meet-logos">
                        <a href="#" target="_self">
                            <img src="./images/home.png" alt="home-logo" width='137' height='44'>
                        </a>
                        <a href="#" target="_self">
                            <img src=" ./images/vs.png" alt="vs-logo" width='107' height='106'>
                        </a>
                        <a href="#" target="_self">
                            <img src="./images/brand.png" alt="brand-logo" width='170' height='44'>
                        </a>
                        <a href="#" target="_self">
                            <img src="./images/nature-home.png" alt="natural-home-logo" width='151' height='46'>
                        </a>
                        <a href="#" target="_self">
                            <img src="./images/classic.png" alt="classic-logo" width='122' height='49'>
                        </a>
                    </div>
                </div>
            </div>

        </div>
    </section>
    <!-- help-section end -->


    <!-- history-section start -->
    <section class="history-section-wrap">
        <div class="container">
            <div class="history-section">
                <div class="history-content-wrap">
                    <div class="lawyer-history">
                        <div class="lawyer-history-wrap">
                            <div class="section-title">
                                <h2 class="h2">
                                    We Are Top Lawyers And History
                                </h2>
                                <img src="./images/practice-icon.png" alt="section-icon" width='95' height='44'>
                            </div>

                            <ul class="lawyer-history-listing">
                                <li>
                                    <div class="history-year">
                                        <div class='history-year-name'>
                                            1995 - Opening
                                        </div>
                                    </div>
                                    <div class="history-content">
                                        <p>
                                            There are many variations of passages
                                            of Lorem Ipsum available, but majority
                                            going to use a passage
                                        </p>
                                    </div>
                                </li>

                                <li>
                                    <div class="history-year">
                                        <div class='history-year-name'>
                                            2011 - Open Branch Offlice
                                        </div>
                                    </div>
                                    <div class="history-content">
                                        <p>
                                            There are many variations of passages
                                            of Lorem Ipsum available, but majority
                                            going to use a passage
                                        </p>
                                    </div>
                                </li>

                                <li>
                                    <div class="history-year">
                                        <div class='history-year-name'>
                                            2015 - 18000K client`s
                                        </div>
                                    </div>
                                    <div class="history-content">
                                        <p>
                                            There are many variations of passages
                                            of Lorem Ipsum available, but majority
                                            going to use a passage
                                        </p>
                                    </div>
                                </li>

                                <li>
                                    <div class="history-year">
                                        <div class='history-year-name'>
                                            1995 - Opening
                                        </div>
                                    </div>
                                    <div class="history-content">
                                        <p>
                                            There are many variations of passages
                                            of Lorem Ipsum available.
                                        </p>
                                    </div>
                                </li>
                            </ul>

                        </div>
                        <div class="history-image-wrap">
                            <img src="./images/history-section/history-wrap.png" alt="history-image" width='264'
                                height='778'>
                        </div>
                    </div>

                    <div class="specialised-wrap">
                        <div class="specialised-content">
                            <div class="section-title">
                                <h2 class="h2">
                                    We Are Specialised And Experienced
                                </h2>
                                <img src="./images/practice-icon.png" alt="section-title" width='95' height='44'>
                            </div>
                            <div class="section-content">
                                <p>
                                    At vero eos et accusamus et iusto odio dignissimos ducimus qui blanditiis
                                    praesentium deleniti atque corrupti quos molestias excepturi odio dignissimos
                                    ducimus qui blanditiis sint occaecati
                                </p>
                            </div>
                        </div>


                        <ul class="experiance-listing">
                            <li>
                                <div class="experiance-wrap">
                                    <div class="experiance-name">
                                        <p>
                                            Divorce And Family Cases
                                        </p>
                                    </div>
                                    <div class="experiance-number">
                                        <p>
                                            75%
                                        </p>
                                    </div>
                                </div>
                                <div class="experiance-progress">
                                    <progress value="75" max="100">

                                    </progress>
                                    <div class="progress-size">

                                    </div>
                                </div>
                            </li>

                            <li>
                                <div class="experiance-wrap">
                                    <div class="experiance-name">
                                        <p>
                                            Property And Construction
                                        </p>
                                    </div>
                                    <div class="experiance-number">
                                        <p>
                                            80%
                                        </p>
                                    </div>

                                </div>
                                <div class="experiance-progress">
                                    <progress value="80" max="100">

                                    </progress>
                                    <div class="progress-size progress-bar">

                                    </div>
                                </div>
                            </li>

                            <li>
                                <div class="experiance-wrap">
                                    <div class="experiance-name">
                                        <p>
                                            Banking and Finance
                                        </p>
                                    </div>
                                    <div class="experiance-number">
                                        <p>
                                            75%
                                        </p>
                                    </div>
                                </div>
                                <div class="experiance-progress">
                                    <progress value="75" max="100">

                                    </progress>
                                    <div class="progress-size">

                                    </div>
                                </div>

                            </li>

                            <li>
                                <div class="experiance-wrap">
                                    <div class="experiance-name">
                                        <p>
                                            Banking and Finance
                                        </p>
                                    </div>
                                    <div class="experiance-number">
                                        <p>
                                            90%
                                        </p>
                                    </div>
                                </div>
                                <div class="experiance-progress">
                                    <progress value="90" max="100">

                                    </progress>
                                    <div class="progress-size  progress">

                                    </div>
                                </div>
                            </li>
                        </ul>


                        <div class="cta-btn btn">
                            <a href="contact.php" href="#" target="_self">
                                Free Consulting
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- history-section end -->

    <!-- client-section start -->
    <section class="client-section-wrap">
        <div class="container">
            <div class="client-section">
                <div class="section-title">
                    <h2 class="h2">Client Opinions & Review</h2>
                    <img src="./images/Icon.png" alt="section-icon" width='95px' height='45px'>
                </div>

                <ul class="client-listing">
                    <li>
                        <div class="client-review">
                            <div class="client-content">
                                <p>
                                    There are of Lorem Ipsum available, but the majority have su alteration in some
                                    form, by injected oir which don't look even slightly believable.
                                </p>
                            </div>
                        </div>

                        <div class="client-details">
                            <div class="client-img">
                                <img src="./images/client-nattasha.png" alt="Nattasha" width='84' height='84'>
                            </div>

                            <div class="client-info">
                                <div class="client-name">
                                    Nattasha
                                </div>
                                <div class="client-edu">
                                    Junior Lawyer
                                </div>
                            </div>
                        </div>
                    </li>

                    <li>
                        <div class="client-review">
                            <div class="client-content">
                                <p>
                                    There are of Lorem Ipsum available, but the majority have su alteration in some
                                    form, by injected oir which don't look even slightly believable.
                                </p>
                            </div>
                        </div>

                        <div class="client-details">
                            <div class="client-img">
                                <img src="./images/client-pall.png" alt="Minci-pall" width='84' height='84'>
                            </div>

                            <div class="client-info">
                                <div class="client-name">
                                    Minci pall
                                </div>
                                <div class="client-edu">
                                    Consultant
                                </div>
                            </div>
                        </div>
                    </li>


                    <li>
                        <div class="client-review">
                            <div class="client-content">
                                <p>
                                    There are of Lorem Ipsum available, but the majority have su alteration in some
                                    form, by injected oir which don't look even slightly believable.
                                </p>
                            </div>
                        </div>

                        <div class="client-details">
                            <div class="client-img">
                                <img src="./images/client-rose.png" alt="Julia-Rose" width='84' height='84'>
                            </div>

                            <div class="client-info">
                                <div class="client-name">
                                    Julia Rose
                                </div>
                                <div class="client-edu">
                                    Consultant
                                </div>
                            </div>
                        </div>
                    </li>


                    <li>
                        <div class="client-review">
                            <div class="client-content">
                                <p>
                                    There are of Lorem Ipsum available, but the majority have su alteration in some
                                    form, by injected oir which don't look even slightly believable.
                                </p>
                            </div>
                        </div>

                        <div class="client-details">
                            <div class="client-img">
                                <img src="./images/client-John.png" alt="John" width='84' height='84'>
                            </div>

                            <div class="client-info">
                                <div class="client-name">
                                    John David
                                </div>
                                <div class="client-edu">
                                    Designer
                                </div>
                            </div>
                        </div>
                    </li>

                </ul>
            </div>
        </div>
    </section>
    <!-- client-section end -->

    <!-- experienc-answer start -->
    <section class="experience-answer-section-wrap">
        <div class="container">
            <div class="experience-title-section">
                <div class="section-title">
                    <h2 class="h2">
                        Our Experienced Attorney Are Ready
                        <br>
                        To answer any questions
                    </h2>
                    <img src="./images/Icon.png" alt="section-icon" width='95' height='44'>
                </div>
            </div>
            <ul class="team-item-listing">
                <li>
                    <a href="#" target="_self">
                        <img src="./images/experience-one.png" alt="experienc-person" width='448' height='593'>
                    </a>

                    <div class="experience-content">
                        <div class="item-content">
                            <div class="team-social-media">
                                <a href='https://www.instagram.com/' target="_blank">
                                    <img src='./images/instagram.png' width='20' height='19' />
                                </a>

                                <a href='https://www.facebook.com/' target="_blank">
                                    <img src='./images/facebook.png' alt='facebook' width='20' height='19' />
                                </a>

                                <a href='https://www.twitter.com/' target="_blank">
                                    <img src='./images/twitter.png' alt='twitter' width='21' height='17' />
                                </a>

                                <a href='https://www.pintrest.com/' target="_blank">
                                    <img src='./images/rest.png' alt='twitter' width='20' height='20' />
                                </a>
                            </div>
                            <div class="team-info">
                                <div class='team-info-name'>
                                    Thomas Daniyel
                                </div>
                                <div class='team-edu'>
                                    Civil Attorney
                                </div>
                            </div>

                        </div>
                    </div>
                </li>

                <li>
                    <a href="#">
                        <img src="./images/experience-three.png" alt="experienc-person" width='448' height='593'>
                    </a>

                    <div class="experience-content">
                        <div class="item-content">
                            <div class="team-social-media">
                                <a href='https://www.instagram.com/' target="_blank">
                                    <img src='./images/instagram.png' width='20' height='19' />
                                </a>

                                <a href='https://www.facebook.com/' target="_blank">
                                    <img src='./images/facebook.png' alt='facebook' width='20' height='19' />
                                </a>

                                <a href='https://www.twitter.com/' target="_blank">
                                    <img src='./images/twitter.png' alt='twitter' width='21' height='17' />
                                </a>

                                <a href='https://www.pintrest.com/' target="_blank">
                                    <img src='./images/rest.png' alt='twitter' width='20' height='20' />
                                </a>
                            </div>
                            <div class="team-info">
                                <div class='team-info-name'>
                                    Nikolona Gail
                                </div>
                                <div class='team-edu'>
                                    Senior Attorney
                                </div>
                            </div>
                        </div>
                    </div>
                </li>

                <li>
                    <a href="#">
                        <img src="./images/experience-two.png" alt="experienc-person" width='448' height='593'>
                    </a>

                    <div class="experience-content">
                        <div class="item-content">
                            <div class="team-social-media">
                                <a href='https://www.instagram.com/' target="_blank">
                                    <img src='./images/instagram.png' width='20' height='19' />
                                </a>

                                <a href='https://www.facebook.com/' target="_blank">
                                    <img src='./images/facebook.png' alt='facebook' width='20' height='19' />
                                </a>

                                <a href='https://www.twitter.com/' target="_blank">
                                    <img src='./images/twitter.png' alt='twitter' width='21' height='17' />
                                </a>

                                <a href='https://www.pintrest.com/' target="_blank">
                                    <img src='./images/rest.png' alt='twitter' width='20' height='20' />
                                </a>
                            </div>
                            <div class="team-info">
                                <div class='team-info-name'>
                                    Michal David
                                </div>
                                <div class='team-edu'>
                                    Criminal Attorney
                                </div>
                            </div>
                        </div>
                    </div>
                </li>
            </ul>
            <div class='team-cta'>
                <div class='cta-btn l-btn'>
                    <a href='testimonials.php' target="_self">
                        Meet Attorney
                    </a>
                </div>
            </div>
        </div>
    </section>
    <!-- experienc-answer end -->

    <!-- consultation-section-start -->
    <section class="consultation-section-wrap">
        <div class="container">
            <div class="consulatation-section">
                <div class="consulatation-left">
                    <div class="consulatation-left-content">
                        <h2 class="h2">
                            Our Expert professional law team is always ready to serve you the best solution!
                        </h2>

                        <p>
                            There are of Lorem Ipsum available, but the majority have su alteration in some form, by
                            injected oir which don't look even slightly believable.
                        </p>

                        <div class='cta-btn btn'>
                            <a href='contact.php' target="_self">
                                Contact us
                            </a>
                        </div>

                    </div>
                </div>
                <div class="consulation-right">
                    <div class="consulation-right-content">
                        <div class="consulation-border">
                            <div class="consulation-content">
                                <h2 class="h2">
                                    Get a Free Consultation
                                </h2>
                                <p>
                                    There are of Lorem Ipsum available, but the majority by injected humour which don't
                                    look even.
                                </p>
                            </div>

                            <div class="consultation-contact">
                                <div class="contact-logo">
                                    <svg width="27" height="27" viewBox="0 0 27 27" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M26.9368 20.379C26.6222 20.0439 25.5114 19.0531 23.4695 17.7422C21.4125 16.4199 19.8955 15.5954 19.4604 15.4032C19.422 15.3862 19.3798 15.3799 19.3381 15.3851C19.2965 15.3903 19.2571 15.4068 19.2241 15.4327C18.5232 15.9793 17.3431 16.9834 17.2798 17.0377C16.8712 17.3878 16.8712 17.3878 16.5367 17.2787C15.9485 17.0859 14.1211 16.1155 12.5288 14.5196C10.9365 12.9237 9.91609 11.0511 9.72323 10.4635C9.61293 10.1284 9.61293 10.1284 9.9643 9.7198C10.0185 9.65651 11.0232 8.47645 11.5699 7.77612C11.5958 7.74314 11.6123 7.70372 11.6175 7.66209C11.6227 7.62047 11.6164 7.57821 11.5994 7.53987C11.4072 7.10412 10.5827 5.58775 9.26036 3.53077C7.94769 1.48946 6.95806 0.378703 6.62296 0.0640988C6.59221 0.0350811 6.55421 0.0148947 6.51295 0.00566677C6.47168 -0.00356118 6.4287 -0.00148897 6.38852 0.0116648C5.21716 0.414193 4.08699 0.927812 3.01343 1.54551C1.97705 2.14797 0.99587 2.84074 0.0813184 3.61575C0.0493935 3.64289 0.0254587 3.67821 0.0120806 3.71792C-0.00129751 3.75763 -0.00361416 3.80023 0.00537896 3.84116C0.131342 4.42818 0.733434 6.87872 2.60179 10.2731C4.50811 13.7373 5.82922 15.5122 8.62873 18.3021C11.4282 21.092 13.2592 22.4926 16.7271 24.3989C20.1215 26.2673 22.5733 26.8699 23.1591 26.9947C23.2001 27.0036 23.2428 27.0013 23.2826 26.9879C23.3224 26.9745 23.3578 26.9506 23.3851 26.9188C24.16 26.0042 24.8526 25.0231 25.4548 23.9867C26.0723 22.9131 26.586 21.7829 26.9886 20.6116C27.0015 20.5717 27.0035 20.5291 26.9944 20.4882C26.9853 20.4473 26.9654 20.4096 26.9368 20.379Z"
                                            fill="white" />
                                    </svg>
                                </div>

                                <div class="contact-details">
                                    <div class="contact-call">
                                        Call Us On:
                                    </div>
                                    <div class="contact-number">
                                        <a href="tel: 987654321" target="_self">
                                            911-987654321
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- consulatation-section-end -->

    <!-- form section start -->
    <section class="form-section-wrap">
        <div class="container">
            <div class="form-section">
                <div class="form">
                    <form>
                        <div class="col-2">
                            <div class="fieldset">
                                <input type="text" placeholder="Full Name *" required />
                            </div>
                            <div class="fieldset">
                                <input type="email" placeholder="Email Address *" required />
                            </div>
                        </div>

                        <div class="col-2">
                            <div class="fieldset">
                                <input type="number" required placeholder="Phone Number *" />
                            </div>
                            <div class="fieldset">
                                <input type="text" required placeholder="Subject *" />
                            </div>
                        </div>

                        <div class="col">
                            <textarea rows="4" cols="50" placeholder='Your Message*' required></textarea>
                        </div>

                        <div class='cta-btn l-btn'>
                            <button>
                                Get An Appoinment
                            </button>
                        </div>

                    </form>
                </div>
            </div>
        </div>
    </section>
    <!-- form section end -->

    <!-- recent-blogs Start -->
    <section class="recent-blogs-section-wrap">
        <div class="container">
            <div class="section-title">
                <h2 class="h2">Recent Blogs</h2>
                <img src="./images/Icon.png" alt="section-icon" width='95' height='44'>
            </div>

            <div class="recent-blogs-section">
                <div class="recent-blog-left">
                    <a href="blog.php" target="_self">
                        <img src="./images/Blog-image.png" alt="blog-image" width='696' height='463'>
                    </a>
                </div>

                <div class="recent-blog-right">
                    <div class="category-blog ">
                        <div class="category-blog-content">
                            <a href="blog-single.php" target="_self">
                                Criminal Law, Kidnapping
                            </a>
                        </div>
                        <div class="category-blog-date">
                            Sunday, July 24, 2022
                        </div>
                    </div>

                    <div class="blog-content">
                        <h2 class='h2'>
                            An Independent Examination Of Charity
                            Accounts
                        </h2>
                        <p>
                            There are of Lorem Ipsum available, but the
                            majority have su alteration in some form, by
                            injected oir which don't look even slightly
                            believable.
                        </p>
                    </div>

                    <div class="cta-btn">
                        <a href="blog-single.php" target="_self">
                            Read More
                        </a>
                    </div>
                </div>
            </div>

            <ul class="blog-listing">
                <li>
                    <div class="blog-img">
                        <a href="business-law.php" target="_self">
                            <img src="./images/blog-one.png" alt="blog-image" width='443' height='334'>
                        </a>
                    </div>

                    <div class="multi-blogs-details">
                        <div class="category-blog ">
                            <div class="category-blog-content">
                                <a href="business-law.php">
                                    Legal Advice
                                </a>
                            </div>
                            <div class="category-blog-date">
                                Sunday, July 24, 2022
                            </div>
                        </div>

                        <div class="blog-content">
                            <div class="blog-name">
                                <h3 class='h3'>

                                    Prevent A Further Breach of The Peace In The States

                                </h3>
                            </div>
                            <p>
                                There are of Lorem Ipsum available,
                                but the majority have su alteration
                                in some form, by injected oir which
                                don't look even slightly believable.
                            </p>

                        </div>

                        <div class="cta-btn">
                            <a href="business-law.php" target="_self">
                                Read More
                            </a>
                        </div>
                    </div>
                </li>

                <li>
                    <div class="blog-img">
                        <a href="#" target="_self">
                            <img src="./images/blog-two.png" alt="blog-image" width='443' height='334'>
                        </a>
                    </div>

                    <div class="multi-blogs-details">
                        <div class="category-blog ">
                            <div class="category-blog-content">
                                <a href="blog-single.php" target="_self">
                                    Legal Advice
                                </a>
                            </div>
                            <div class="category-blog-date">
                                Sunday, July 24, 2022
                            </div>
                        </div>

                        <div class="blog-content">
                            <div class="blog-name">
                                <h3 class='h3'>

                                    Failing to Carry Out The Terms of A Business Contract

                                </h3>
                            </div>

                            <p>
                                There are of Lorem Ipsum available,
                                but the majority have su alteration
                                in some form, by injected oir which
                                don't look even slightly believable.
                            </p>

                        </div>

                        <div class="cta-btn">
                            <a href="blog-single.php" target="_self">
                                Read More
                            </a>
                        </div>
                    </div>
                </li>

                <li>
                    <div class="blog-img">
                        <a href="blog-single.php" target="_self">
                            <img src="./images/blog-three.png" alt="blog-image" width='443' height='334'>
                        </a>
                    </div>

                    <div class="multi-blogs-details">
                        <div class="category-blog ">
                            <div class="category-blog-content">
                                <a href="blog-single.php" target="_self">
                                    Legal Advice
                                </a>
                            </div>
                            <div class="category-blog-date">
                                Sunday, July 24, 2022
                            </div>
                        </div>

                        <div class="blog-content">
                            <div class="blog-name">
                                <h3 class='h3'>
                                    How Legal Professionals Work: The Defense Objects In The Event
                                </h3>
                            </div>

                            <p>
                                There are of Lorem Ipsum available,
                                but the majority have su alteration
                                in some form, by injected oir which
                                don't look even slightly believable.
                            </p>

                        </div>

                        <div class="cta-btn">
                            <a href="blog-single.php" target="_self">
                                Read More
                            </a>
                        </div>
                    </div>
                </li>

                <li>
                    <div class="blog-img">
                        <a href="business-law.php" target="_self">
                            <img src="./images/blog-four.png" alt="blog-image" width='443' height='334'>
                        </a>
                    </div>

                    <div class="multi-blogs-details">
                        <div class="category-blog ">
                            <div class="category-blog-content">
                                <a href="business-law.php" target="_self">
                                    Legal Advice
                                </a>
                            </div>
                            <div class="category-blog-date">
                                Sunday, July 24, 2022
                            </div>
                        </div>

                        <div class="blog-content">
                            <div class="blog-name">
                                <h3 class='h3'>
                                    Riding solo to the rescue of her beloved nonprofits
                                </h3>
                            </div>

                            <p>
                                There are of Lorem Ipsum available,
                                but the majority have su alteration
                                in some form, by injected oir which
                                don't look even slightly believable.
                            </p>

                        </div>

                        <div class="blog-more-details">
                            <div class="cta-btn">
                                <a href="blog.php" target="_self">
                                    Read More
                                </a>
                            </div>
                        </div>
                    </div>
                </li>

                <li>
                    <div class="blog-img">
                        <a href="business-law.php" target="_self" class="text-link">
                            <img src="./images/blog-five.png" alt="blog-image" width='443' height='334'>
                        </a>
                    </div>

                    <div class="multi-blogs-details">
                        <div class="category-blog ">
                            <div class="category-blog-content">
                                <a href="blog.php" target="_self">
                                    Legal Advice
                                </a>
                            </div>
                            <div class="category-blog-date">
                                Sunday, July 24, 2022
                            </div>
                        </div>

                        <div class="blog-content">
                            <div class="blog-name">
                                <h3 class='h3'>
                                    Case Filed on Behalf of Injured Tugboat Worker
                                </h3>
                            </div>

                            <p>
                                There are of Lorem Ipsum available,
                                but the majority have su alteration
                                in some form, by injected oir which
                                don't look even slightly believable.
                            </p>

                        </div>

                        <div class="cta-btn">
                            <a href="blog.php" target="_self">
                                Read More
                            </a>
                        </div>
                    </div>
                </li>

                <li>
                    <div class="blog-img">
                        <a href="blog.php" target="_self">
                            <img src="./images/blog-six.png" alt="blog-image" width='443' height='334'>
                        </a>
                    </div>

                    <div class="multi-blogs-details">
                        <div class="category-blog ">
                            <div class="category-blog-content">
                                <a href="business-law.php" target="_self">
                                    Legal Advice
                                </a>
                            </div>
                            <div class="category-blog-date">
                                Sunday, July 24, 2022
                            </div>
                        </div>

                        <div class="blog-content">
                            <div class="blog-name">
                                <h3 class='h3'>

                                    How Companies Choose Legal Representation

                                </h3>
                            </div>
                            <p>
                                There are of Lorem Ipsum available,
                                but the majority have su alteration
                                in some form, by injected oir which
                                don't look even slightly believable.
                            </p>

                        </div>

                        <div class="cta-btn">
                            <a href="blog.php" target="_self">
                                Read More
                            </a>
                        </div>
                    </div>
                </li>
            </ul>
        </div>
        </div>
    </section>
    <!-- recent-blogs end -->

    <!-- Image Section start -->
    <section class="image-section">
        <img src="./images/section-img.png" alt="Image Section" width='1920' height='330'>
    </section>
    <!-- Image Section start -->

    <!-- footer section start -->
    <section class="footer-wrap">
        <div class="container">
            <footer>
                <div class='footer-top'>
                    <div class="footer-logo">
                        <a href='index.php' target="_self">
                            <img src="./images/attorneystar.png" alt="Attorneyster" width='398' height='79'>
                        </a>
                    </div>

                    <div class='footer-cols'>
                        <div class='footer-col quick'>
                            <div class='footer-link-title'>
                                Quick Link
                            </div>

                            <ul>
                                <li><a href='index.php' target="_self">Home</a></li>

                                <li><a href='#' target="_self">Lawyer Single</a></li>
                                <li><a href='about-us.php' target="_self">About</a></li>
                                <li><a href='case-result.php' target="_self">Case Results</a></li>
                                <li><a href='contact.php' target="_self">Contact</a></li>
                                <li><a href='practice-area.php' target="_self">Practice Area</a></li>
                                <li><a href='blog.php' target="_self">Blog</a></li>
                                <li><a href='pricing.php' target="_self">Packages</a></li>
                                <li><a href='blog-single.php' target="_self">Blog Post</a></li>
                                <li><a href='#' target="_self">Package Single</a></li>
                                <li><a href='#' target="_self">Lawyers</a></li>
                                <ul>
                        </div>

                        <div class='footer-col'>
                            <div class='footer-link-title'>
                                Utility Page
                            </div>

                            <ul>
                                <li><a href='#' target="_self">Start Here</a></li>
                                <li><a href='#' target="_self">Style Guide</a></li>
                                <li><a href='404.php' target="_self">404 Not Found</a></li>
                                <li><a href='password-protection.php' target="_self">Password Protected</a></li>
                                <li><a href='licenses.php' target="_self">Licenses</a></li>
                                <li><a href='changelog.php
                                ' target="_self">Changelog</a></li>
                            </ul>
                        </div>

                        <div class='footer-col'>
                            <div class='footer-link-title'>
                                Practice Area
                            </div>

                            <ul>
                                <li><a href='#' target="_self">Family Law</a></li>
                                <li> <a href='#' target="_self">Criminal Law</a></li>
                                <li><a href='cases-single.php' target="_self">Parsonal Injury</a></li>
                                <li><a href='#' target="_self">Real Estate Law</a></li>
                                <li><a href='business-law.php' target="_self">Business Law</a></li>
                            </ul>
                        </div>


                        <div class='footer-col contact'>
                            <div class='footer-link-title'>
                                Contact Us
                            </div>

                            <ul>
                                <li>
                                    <a href='https://www.google.com/maps/search/Head+Office+Address+121+King+Street,+Melbourne+West,+Australia/@-37.8172467,144.9532001,17z/data=!3m1!4b1?entry=ttu'
                                        target="_blank">
                                        Head Office Address 121 King Street, Melbourne West, Australia
                                    </a>
                                </li>
                                <li>
                                    <p>Phone: </p>
                                    <a href="tel: 987654321" target="_self">
                                        (011) 987654321
                                    </a>
                                </li>
                                <li>
                                    <p>Email: </p>
                                    <a href='mailto:info@example.com' target="_self">
                                        Info@Example.com
                                    </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>

                <div class='footer-bottom'>
                    <div class='footer-bottom-left'>
                        <div class='footer-copyright'>
                            Copyright &copy; <span>Attorney Law</span>
                        </div>
                        <div class='footer-design'>
                            Designed by <span>VictorFlow</span> Templates - Powered by WebFlow
                        </div>
                    </div>

                    <div class='footer-bottom-right'>
                        <div class="footer-follow">
                            Follow:
                        </div>
                        <ul>
                            <li>
                                <a href='https://www.instagram.com/' target="_blank">
                                    <img src='./images/Insta.png' alt='instagram' width='19' height='19'>
                                </a>
                            </li>

                            <li>
                                <a href='https://www.facebook.com/' target="_blank">
                                    <img src='./images/Fb.png' alt='facebook' width='19' height='19'>
                                </a>
                            </li>

                            <li>
                                <a href='https://www.twitter.com/' target="_blank">
                                    <img src='./images/Twiter.png' alt='twitter' width='19' height='19'>
                                </a>
                            </li>

                            <li>
                                <a href='https://www.pintrest.com/' target="_blank">
                                    <img src='./images/Pintrest.png' alt='pintrest' width='19' height='19'>
                                </a>
                            </li>
                        </ul>
                    </div>
                </div>
            </footer>

        </div>
    </section>
    <!-- footer section end -->
</body>

</html>